...
\newcommand {\DrawInputsA}[4][0] {
	...
	\if \ShowLateData 1
		...
		\if \IsGhost 0
			...
			\fi
		\fi
		\DrawInput[\LateNineBorder]{\LateNineColor}{3.0}{2.666 + #1}{4}{#3}{D}
...
\newcommand{\DrawPerfectWatermarkA}[1] {
	\draw [-, s_watermark, #1]
	(\ox + 0.111, \oy + 0.01)
	to [out=80,in=200] (\ox + 0.5, \oy +  .7)
	to [out=20,in=250] (\ox + 1.15, \oy +  1.3)
	to [out=70,in=225] (\ox + 2.0, \oy +  1.8)
	to [out=45,in=210] (\ox + 3, \oy +  3)
	to [out=30,in=230] (\ox + 6.6, \oy +  4.3)
	to [out=50,in=180] (\ox + 7, \oy +  4.6)
...
\newcommand{\DrawHeuristicWatermarkA}[1] {
	...
	to [out=0, in=180] (\ox + 8, \oy +  2.25)
	to [out=0, in=180] (\ox + 8.75, \oy +  2.75)
	to [out=0, in=180] (\ox + 12.000, \oy +  2.9) % end
	;
}
...
\def \ShowLateness{0}
\def \latey{.1}
\newcommand{\DrawLatenessHorizonText}[5]{
	\contourlength{0.66pt}
	\node[font=\bf\sffamily\scriptsize,c_outlined_text] at (\ox + #2 + \lateness, \oy +  #3 +.52) {\contour{c_outlined_text_outline}{[12:0#4, 12:0#5)}};
...
\newcommand{\DrawTombstone}[6]{
	...
	\DrawLatenessHorizonText{#1}{#2}{#4}{#5}{#6}
}

...
parser.add_argument('-m', '--mode', action='store', dest='mode', default='both',help='Operation modes: %s'%str(allowed_actions))
# parser.add_argument('-M', '--message', action='store', dest='message', default='Hello World!',
#                     help='Message to publish')
parser.add_argument('--region', action='store', dest='region', default='ap-southeast-1')
parser.add_argument('--max-pub-ops', action='store', dest='max_pub_ops', default=100)
...
thing_name = 'lab3rpi'
topic = 'clients/rpi-50046/hello/world'

io.init_logging(getattr(LogLevel, "Warn"), 'stderr')
...
if args.mode == 'both' or args.mode == 'subscribe':
    ...
    subscribe_future, _ = mqtt_connection.subscribe(topic, QoS.AT_MOST_ONCE, on_publish)
...
while loop_count < args.max_pub_ops:
    if args.mode == 'both' or args.mode == 'publish':
        ...
        message['message'] = str(randomNum)
        message['sequence'] = loop_count
        messageJson = json.dumps(message)
        pub_future, _ = mqtt_connection.publish(topic, str(randomNum).encode(), QoS.AT_MOST_ONCE)
        pub_future.result()
        print('Published topic {}: {}\n'.format(topic, str(randomNum).encode()))
